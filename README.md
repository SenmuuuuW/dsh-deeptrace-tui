...
----------------------------------------------------------------------------------------------------
成本 ¥7.04（▼ 91%）   会话 97  回合 23  Tokens 112.67M                 ░
Cache 99.8%  夜间 11%                                                  ▒▒▓▓
                                                                  ▓▓▓ ▓▓▓▓▓▓
趋势 · TREND（近 5 周期）                                          ▓▓▓▓▓▓▓█▓▓
成本  W30   ¥0.00  ▁▁▁▁▁██▂                                     █████▓▓▓▓▓▓▓▓███
...
Cache  W32  95.5%  ▁▁▁▁████                                           ▓▓▓▓
Cache  W33  99.4%  ▁▁▁▁████                                            ▓▓
Cache LIVE  99.8%  ▁▁▁▁████  ○ LIVE                         鲸评 WHALE NOTE / OBSERVER
夜间  W30   0%  ▁▁▁▁▁██▇                                    （摆摆尾巴）嗨，我来啦。
夜间  W31   0%  ▁▁▁▁▁██▇                                    呜哇——这期的危险操作，有点多哦。
夜间  W32   0%  ▁▁▁▁▁██▇                                    （认真检查）删库、强推、格式化……你是想给
夜间  W33  13%  ▁▁▁▁▁██▇                                    运维上强度吗？
夜间 LIVE  11%  ▁▁▁▁▁██▇  ○ LIVE                            重要目录记得先备份，这个真的不是开玩笑的
                                                            。
发现 · FINDINGS（本期洞察）                                 下次动手之前，先让我看一眼，好不好？
01 提示 检测到 7 次重试风暴                                 安全第一，我们一起把项目养得好好的。
02 信息 缓存命中率 99.8%                                    我数着数着，都快给你配上背景音乐了。
03 提示 3 条需留意操作                                      以上，就是本期小评。
04 提示 会话碎片化：平均每会话仅 0.2 回合
05 提示 费用较上一周期下降 91%
06 警告 工具 edit 失败 60 次，失败率 10%
  同一命令连续重复 ≥3 次，共 7 次，占总命令约 10%。
  建议：检查失败命令的前置条件（路径/依赖/权限）。
----------------------------------------------------------------------------------------------------
[j/k] 移动  [Enter] 跳转  [r] 刷新  [?] 帮助  [q] 退出       更新于 20:43:01 · 97 存档 · 78,933 事件
```
...
```
工具健康 TOOL HEALTH · 按关注度排序（异常优先）· 共 24 个工具 · 调用 350 · 失败 12
01 edit 成功 90.0% · 调用 601 · 失败 60 平均 67ms · P95 93ms
█████████░  FS_NOT_OBSERVED ×30  FS_STALE_VERSION ×21  FS_EDIT_NOT_FOUND ×9
02 bash 成功 99.6% · 调用 3641 · 失败 15 · 未配对 1 平均 6.7s · P95 38.3s
██████████  TOOL_OUTCOME_UNKNOWN ×15
03 write 成功 96.8% · 调用 400 · 失败 13 平均 106ms · P95 89ms
██████████  FS_NOT_OBSERVED ×7  FS_STALE_VERSION ×5  FS_SANDBOX_DENIED ×1
04 web_search 成功 99.0% · 调用 383 · 失败 4 平均 11.4s · P95 20.4s
██████████  TOOL_TIMEOUT ×3  WEB_PROVIDER_ERROR ×1
05 ask_user_question 成功 88.9% · 调用 27 · 失败 3 平均 270.3s · P95 1479.7s
█████████░  ASK_CANCELLED ×2  TOOL_OUTCOME_UNKNOWN ×1
06 structured_output 成功 90.5% · 调用 21 · 失败 2 平均 12ms · P95 17ms
▼ 更多```
...
```
按费用排序 · Top 20 / 共 97 会话
01 ¥33.14 26 RETRY 7 RISK   573.47M tokens · 1423 工具
     去github dsh-plugin 和dsh topic底下  08-17 18:30
02 ¥13.92 9 RETRY 14 RISK   239.21M tokens · 626 工具
     你现在位于一个全新的 Git repos  08-10 21:40
03 ¥13.47 10 RETRY 1 RISK   227.18M tokens · 553 工具
     我想为 DeepSeek Harness 设计并开  08-12 21:20
04 ¥10.35 14 RETRY   157.87M tokens · 495 工具
```

Enter 展开会话详情（费用 / Tokens / 重试 / 风险 / 主要指标），`c` 复制 Session ID——**不伪造跳转能力**（官方暂无跨应用"打开会话"机制，与 Web 版一致）。
## 鲸鱼娘 WHALE

DeepTrace 的观察员 —— **chibi Q 版像素鲸鱼娘**：蓝黑头发 + 浅蓝高光、大眼睛小嘴、
鲸鱼帽（背鳍 + 后翘尾）+ 头侧小鳍，圆润轮廓，同一角色只换表情。6 个状态与
`dsh-whale-report/core` 的 `whaleMood()` 规则严格同源：

| sprite | 状态 | 关键特征 |
| --- | --- | --- |
| `idle` | 待机 | 大眼双高光、平静小嘴、腮红 |
| `happy` | 开心 | ∩ 闭眼、ω 嘴、帽顶喷水 |
| `thinking` | 思考 | 上视眼、小 o 嘴、「?」气泡 |
| `warning` | 提醒 | 平眉、平嘴、汗珠（无语态，对应 core dazed） |
| `angry` | 生气 | 外高内低斜眉、撇嘴（仍然可爱） |
| `sleepy` | 困困 | 闭眼线、O 嘴、z z z |

渲染：Unicode half-block 双色（truecolor / 256 降级）+ no-color 亮度剪影；
小终端自动隐藏 / 只显示头部。素材规格与替换方式见 [docs/WHALE_ASSET_SPEC.md](docs/WHALE_ASSET_SPEC.md)。
